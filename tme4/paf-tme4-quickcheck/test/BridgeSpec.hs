...
genBridgeFree = do 
    ...
    nbTo <- choose (0, 50)
    nbFrom <- choose (0, 50)
    lim <- choose (nbI + nbTo + nbFrom, 150)  -- la limite modifier pour avoir des pont valide (connais pas encore comment ajouter un "error" dans le test)
    return $ if nbI + nbTo + nbFrom == lim
             then BridgeClosed lim nbTo nbFrom
             else if nbI + nbTo + nbFrom < lim 
             then BridgeOpened lim nbTo nbI nbFrom
             else undefined
