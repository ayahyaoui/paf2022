...
import MindEngine
--import qualified Data.Text.Internal.Builder as Seq
--import Text.Parsec (Consumed(Empty))
import qualified Data.Sequence as Text
import qualified Data.Sequence as Seq
import qualified Data.Text as DataText
import Data.Text (pack)
import qualified Data.Sequence as Seq
import MindEngine (Secret, Peg)
--import qualified Distribution.Compat.Newtype as Text
...
{-
texToSeq :: Text -> Seq
texToSeq Text.Empty = Seq.Empty
texToSeq t = Text.head t Seq.:>| texToSeq Text.Tail t
-}

--tt :: (Seq -> Char -> Seq ) -> Seq -> Text -> Seq 

allColor = [Black, Blue, Green, Yellow, Cyan, White, Magenta, Red] :: [Peg]

getMachineSecret :: Seq Peg
getMachineSecret =  Seq.fromList  $ take 4  (map (\x -> allColor !! x) (Rd.randomRs (0, length allColor - 1) (Rd.mkStdGen 137) ))

-- todo creer une chaine aleatoire (au lieux de yygb)
prepareSecret :: Role -> IO (Maybe Secret)
prepareSecret Computer = do
  print getMachineSecret -- sroll to cheat
  putStr $ replicate 30 '\n'
  return $ Just (mkSecret getMachineSecret)
